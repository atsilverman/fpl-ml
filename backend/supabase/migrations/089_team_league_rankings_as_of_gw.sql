-- Add optional p_as_of_gw to get_team_league_rankings for rank-change comparisons.
-- When provided, rankings are computed as of that gameweek (e.g. GW28 vs GW29).
-- When NULL, uses current gameweek (existing behavior).

DROP FUNCTION IF EXISTS get_team_league_rankings(text, text);
DROP FUNCTION IF EXISTS get_team_league_rankings(text, text, int);

CREATE OR REPLACE FUNCTION get_team_league_rankings(p_gw_filter TEXT, p_location TEXT, p_as_of_gw INT DEFAULT NULL)
RETURNS TABLE(
  team_id INT,
  table_position INT,
  points INT,
...
DECLARE
  ...
  v_min_gw INT;
BEGIN
  -- Use p_as_of_gw when provided, else current gameweek
  SELECT COALESCE(p_as_of_gw, (SELECT id FROM gameweeks WHERE is_current = true LIMIT 1), 1) INTO v_current_gw;
...
COMMENT ON FUNCTION get_team_league_rankings(TEXT, TEXT, INT) IS
'Team league rankings: PL table position + ranks for goals, xG, GC, xGC. p_as_of_gw: optional, rankings as of that gameweek (for rank-change comparisons).';

GRANT EXECUTE ON FUNCTION get_team_league_rankings(TEXT, TEXT, INT) TO anon, authenticated;
