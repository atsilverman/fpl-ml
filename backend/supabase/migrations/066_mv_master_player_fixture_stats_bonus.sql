-- Add bonus, provisional_bonus, bonus_status and effective_bonus to mv_master_player_fixture_stats
-- so the fixtures API can return bonus for BpsLeadersChart (colored bars and +1/+2/+3).
...
SELECT
  ...
  COALESCE(pgs.bonus, 0)::INTEGER AS bonus,
  COALESCE(pgs.provisional_bonus, 0)::INTEGER AS provisional_bonus,
  pgs.bonus_status,
  (CASE WHEN pgs.bonus_status = 'confirmed' OR COALESCE(pgs.bonus, 0) > 0 THEN COALESCE(pgs.bonus, 0) ELSE COALESCE(pgs.provisional_bonus, 0) END)::INTEGER AS effective_bonus,
