-- RPC: Official Premier League table position + ranks for goals, xG, GC, xGC (1-20).
-- Used by Team Detail Modal. Table position uses PL rules: points (3 win, 1 draw), then goal diff, then goals for.
-- All stats season-to-date (through current gameweek). gw_filter: all|last6|last12, location: all|home|away.
...
BEGIN
  ...
  table_stats AS (
    SELECT
      ff.tid,
      SUM(CASE WHEN ff.gf > ff.ga THEN 3 WHEN ff.gf = ff.ga THEN 1 ELSE 0 END)::INT AS pts,
      SUM(ff.gf)::INT AS gf,
      SUM(ff.ga)::INT AS ga,
      (SUM(ff.gf) - SUM(ff.ga))::INT AS gd
  ...
  xgc_from_def_gk AS (
    SELECT
      d.tid,
      COALESCE(SUM(d.xgc), 0)::NUMERIC AS xgc
    FROM (
      SELECT
        pgs.team_id AS tid,
        pgs.gameweek,
...
COMMENT ON FUNCTION get_team_league_rankings(TEXT, TEXT) IS
'Team league rankings: PL table position (points, GD, GF) + ranks 1-20 for goals, xG, GC, xGC. For Team Detail Modal.';
