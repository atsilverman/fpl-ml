-- Prefer player-derived gameweek_points (includes provisional/official bonus) in standings
-- so league page shows same bonus-inclusive values as home bento (42 vs 35, 1635 vs 1628).
-- Derive total_points from baseline + that gameweek_points so total also includes bonus.
...
base AS (
  SELECT
    ...
    m.manager_team_name,
    mgh.gameweek,
    -- Prefer player-derived GW (includes provisional/official bonus); fallback to history
    COALESCE(pgp.points, NULLIF(mgh.gameweek_points, 0), 0) AS gameweek_points,
    -- Total = baseline + displayed GW so total includes bonus; fallback to mgh when no baseline
    (COALESCE(mgh.baseline_total_points, mgh.total_points - COALESCE(mgh.gameweek_points, 0)) + COALESCE(pgp.points, NULLIF(mgh.gameweek_points, 0), 0))::INTEGER AS total_points,
    ...
    ROW_NUMBER() OVER (
      ...
      ORDER BY (COALESCE(mgh.baseline_total_points, mgh.total_points - COALESCE(mgh.gameweek_points, 0)) + COALESCE(pgp.points, NULLIF(mgh.gameweek_points, 0), 0)) DESC, m.manager_id ASC
    ) AS calculated_rank,
...
'League standings by total points. gameweek_points and total_points prefer player-derived values (v_manager_player_gameweek_points) so provisional/official bonus is shown consistently with home bento.';
