-- Fix "column reference team_id is ambiguous" in get_team_goals_from_fixtures.
-- RETURNS TABLE column names become plpgsql variables; use #variable_conflict use_column
-- so unqualified names resolve to table columns, avoiding shadowing.
-- Re-applies in case 085 was not run or a different PG version needs this.
...
COMMENT ON FUNCTION get_team_goals_from_fixtures(TEXT, TEXT) IS 'Team G, GC, and games_played from fixtures table. gw_filter: all|last6|last12, location: all|home|away. Fixes ambiguous team_id.';
